...
const ManageTokens = async () => {
  ...
  const component = {
    template: `
      <div class="flex flex-col gap-4 p-8" style="width: 100%; max-width: 720px; margin: 0 auto; box-sizing: border-box;">
        <div class="flex items-center justify-between gap-12 py-0">
          <div class="ml-6">
        ...
        <div v-if="!tokens.length" class="flex flex-col items-center gap-4 py-2">
          <Icon icon="profiles" :size="30" class="opacity-40" />
          <div class="text-14">还没有添加账号</div>
          <div class="text-12 opacity-60">在下方粘贴 Token，即可添加第一个签到账号</div>
        </div>
        <div v-else class="flex flex-col gap-6" style="max-height: 320px; overflow-y: auto">
  ...
  style.id = 'refresh-token-modal-style'
  style.textContent = `
    div[data-title="管理 RefreshToken"] .gui-modal-header {padding:10px!important;}
    div[data-title="管理 RefreshToken"] .gui-scroll-view {padding:6px 2px 0 5px!important;}
    .gui-card div[class*="__state--selected"] {display: none !important;}`
...
const capacity = async (accessToken) => {
  ...
  const body = await post('https://api.aliyundrive.com/adrive/v1/user/driveCapacityDetails', { isReward: false }, accessToken)
  // 阿里云盘部分接口出错时会返回 code 或 error 字段，而不是 success
  if (body.code || body.error) {
    throw new Error(body.message || body.error || '获取容量失败')
  }
  // 直接从 body 中解构，并赋予默认值 0 防止字段缺失导致报错
  const { drive_total_size = 0, drive_used_size = 0 } = body
  // 调用 fmt 函数并格式化字符串
  const messages = [`获取容量，总容量: ${fmt(drive_total_size)}，已用容量: ${fmt(drive_used_size)}`]
  return messages
}
const SignIn = async () => {
  ...
  const getDisplayUserName = (name) => {
    if (name === '176***696') return '136***6775'
    if (name === '182***040') return '132***9014'
    return name
  }
  const results = []
  for (const [index, token] of tokens.entries()) {
    ...
    await Plugins.WriteFile(TOKEN_CONFIG, JSON.stringify(tokenMap, null, 2))
    try {
      results.push(`账号【${nick_name}】(${user_name}) ${await signIn(account.access_token)} ${await capacity(account.access_token)}`)
    } catch (error) {
      results.push(`账号【${nick_name} ${user_name}】${error.message || error}`)
    }
  }
  return results
}

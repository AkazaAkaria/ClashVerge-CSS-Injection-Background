...
export default (Plugin) => {
  ...
  class NodeHealth {
    ...
    record(results, services) {
      ...
      this.successRatio = this.lastChecked === 0 ? currentRatio : 0.75 * currentRatio + 0.25 * this.successRatio
  ...
  class SmartPolicyManager {
    ...
    evaluateSwitch() {
      ...
      if (!current) {
        this.switchTo(best, '当前节点不在候选池，切换到最佳节点')
        return
      }
      ...
      if (isCurrentDead) {
        this.switchTo(best, `当前节点故障(可用率 ${Math.round(current.successRatio * 100)}%)，触发紧急自愈切换`)
        return
      }
      ...
      if (bestScore >= currentScore + this.options.hysteresisMargin) {
        this.switchTo(best, `综合分数提升 ${Math.round(bestScore - currentScore)}`)
      } else {
    ...
    switchTo(node, reason) {
      ...
      Plugins.handleUseProxy(groupProxy, nodeProxy)
      this.current = node
    ...
    const component = defineComponent({
      ...
      setup() {
        ...
        onMounted(() => {
          ...
          modalSyncTimer = setInterval(() => syncManagersFromKernel(true, true), 1000)
        })
        onUnmounted(() => {
          if (modalSyncTimer) {
            clearInterval(modalSyncTimer)
